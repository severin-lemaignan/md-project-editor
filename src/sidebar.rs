...
use gtk4::prelude::*;
use gtk4::{
    gio, Box as GtkBox, DirectoryList, Label, ListView, ScrolledWindow, SignalListItemFactory,
    SingleSelection,
};
        ...
        for i in 0..sel.n_items() {
            if let Some(obj) = sel.item(i) {
                if let Ok(info) = obj.downcast::<gio::FileInfo>() {
                    ...
                    if let Some(abs) = info.attribute_string("academic-assistant::absolute_path") {
...
impl Sidebar {
    pub fn new(
        ...
        let dir_list = DirectoryList::new(
            Some("standard::name,standard::icon,standard::type,time::modified"),
            Some(&current_dir),
        );
        ...
        let sort_model_dirs =
            gtk4::SortListModel::new(Some(dir_list.clone()), Some(sorter.clone()));
        ...
        let sort_model_search =
            gtk4::SortListModel::new(Some(search_store.clone()), Some(sorter.clone()));
        ...
        factory.connect_setup(move |_, list_item| {
            ...
            let get_path = move || -> Option<PathBuf> {
                if let Some(obj) = item_for_path.item() {
                    if let Ok(info) = obj.downcast::<gio::FileInfo>() {
                        if let Some(abs) =
                            info.attribute_string("academic-assistant::absolute_path")
                        {
                        ...
                        if std::fs::rename(&path_clone, &new_path).is_ok() {
                            ...
                            if is_current {
                                *current_c.borrow_mut() = Some(new_path.clone());
                                crate::file_ops::update_window_title(
                                    &win_c,
                                    &buf_c,
                                    Some(&new_path),
                                );
            ...
            if let Some(obj) = item.item() {
                if let Ok(info) = obj.downcast::<gio::FileInfo>() {
                    ...
                    if info.file_type() != gio::FileType::Directory
                        ...
                        && crate::file_ops::document_kind_for_name(&name)
                            ...
                            if file_name.contains(&text) {
                                ...
                                if let Ok(metadata) = std::fs::metadata(path) {
                                    if let Ok(sys_time) = metadata.modified() {
                                        ...
                                        {
                                            if let Ok(dt) = gtk4::glib::DateTime::from_unix_utc(
                                ...
                                info.set_attribute_string(
                                    "academic-assistant::absolute_path",
        ...
        list_view.connect_activate(move |_lv, position| {
            if let Some(obj) = sel_for_activate.item(position) {
                if let Ok(info) = obj.downcast::<gio::FileInfo>() {
                    ...
                    let is_search_result = info
                        .attribute_string("academic-assistant::absolute_path")
                        .is_some();
                    if let Some(abs) = info.attribute_string("academic-assistant::absolute_path") {
                        file_path = Some(std::path::PathBuf::from(abs.as_str()));
                    ...
                    if let Some(path) = file_path {
                        if is_dir {
                            ...
                            let _ = gio::Settings::new("org.skadge.academicassistant")
                                .set_string("last-folder", &path.to_string_lossy());
                            ...
                            if is_search_result {
                                if let Some(parent) = path.parent() {
                                    ...
                                    let _ = gio::Settings::new("org.skadge.academicassistant")
                                        .set_string("last-folder", &parent.to_string_lossy());
                                    search_entry_for_activate.set_text("");
                                }
                            }

                            let _ = gio::Settings::new("org.skadge.academicassistant")
                                .set_string("last-file", &path.to_string_lossy());
